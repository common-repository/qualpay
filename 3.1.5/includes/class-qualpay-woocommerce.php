...
class WC_Gateway_Qualpay extends WC_Payment_Gateway_CC
{
    ...
    public function __construct()
    {
        ...
        $this->ach_on = 'yes' === $this->get_option('ach_on', 'yes');
    ...
    public function form()
    {
        ...
        if ($Cart_total > 0 || (Qualpay_Cart::recurring_in_cart())) {
            ...
            if (!$transient_key) {
                ...
                echo '<input type="hidden" id="qualpay_type_id" name="qualpay_type_id" />';
                echo '<input type="hidden" id="qualpay_card_type" name="qualpay_card_type" />';
                echo '<input type="hidden" id="capture_id" name="capture_id" value="' . $this->capture . '" />';
                echo '<input type="hidden" id="achOnOff" name="achOnOff" value="' . $this->ach_on . '" />';
                echo $payment_cards;
                $ach_div = '';
    ...
    public function enqueue_scripts()
    {
        ...
        if ($Cart_total > 0 || (Qualpay_Cart::recurring_in_cart())) {
            ...
            if ($transient_key) {
                echo '<style>'.str_replace(array("\n\r", "\n", "\r"), "", $this->get_option('custom_css')).'<style>';
        ...
        $this->form_fields = apply_filters('qualpay_wc_settings',
            array(
                ...
                'capture' => array(
                    ...
                    'type' => 'checkbox',
                    'description' => __('Whether or not to immediately capture the charge. When unchecked, the charge issues an authorization and will need to be captured later. All recurring, one-time, and ACH payments will be captured immediately.', 'qualpay'),
                    'default' => 'yes',
                    'desc_tip' => true,
                ),
                'ach_on' => array(
                    'title' => __('ACH', 'qualpay'),
                    'label' => __('Enable  ACH Payments', 'qualpay'),
                    'type' => 'checkbox',
                    'description' => __('Qualpay ACH Payments requires a separate merchant account application.  Contact Qualpay at <a href="mailto:sales@qualpay.com">sales@qualpay.com</a> if you need to apply for ACH Payments.', 'qualpay'),
                    'default' => '',
                    'desc_tip' => true
                ),
                'recurring' => array(
    ...
    //     echo " validate Checkout from qualpay";
    ...
    {   
        ...
        $billing_company = version_compare(WC_VERSION, '3.0.0', '<') ? $order->billing_company : $order->get_billing_company();
        
        
        if(isset($_POST['qualpay_card_type'] ) && $_POST['qualpay_card_type'] == 'AP') {
            if(isset($_POST['qualpay_type_id'] ) && ($_POST['qualpay_type_id'] == 'K'  || $_POST['qualpay_type_id'] == 'V') && $billing_company != '') {
                $post_data['cardholder_name'] = $billing_company; 
            } else {
